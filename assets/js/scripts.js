...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 경로에 따라 컨텐츠 로드
    if (path === '/report/daily_group') {
        document.getElementById('subtitle').textContent = '일자별 레포트';
        fetchReportsByDate();
    } else {
        document.getElementById('subtitle').textContent = '최근 게시된 레포트';
        fetchRecentReports();
    }
});
...
function fetchReportsByDate() {
    ...
    fetch('/static/reports/grouped_reports.json')
...
function fetchRecentReports() {
    document.getElementById('loading').style.display = 'block';
    fetch('/static/reports/recent_reports.json')
        .then(response => response.json())
        .then(data => {
            document.getElementById('loading').style.display = 'none';
            displayReports(data);
